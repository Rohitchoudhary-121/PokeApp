...
namespace PokemonWebApp.Application.Models.Common
{
    public class BaseResult
    {
        ...
        public void AddExceptionLog(Exception ex)
        {
            ...
            else if (!IsInDebuggingMode)
            {
                const string COMMON_ERROR_MESSAGE = "Something went wrong, Please try again later.";
                if (Errors == null) // Initialize if needed
                    Errors = new List<string>();

                if (!Errors.Contains(COMMON_ERROR_MESSAGE))
                    Errors.Add(COMMON_ERROR_MESSAGE);
            }
        }
    }
}
